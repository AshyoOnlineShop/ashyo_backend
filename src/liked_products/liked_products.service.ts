...
export class LikedProductsService {
  constructor (@InjectModel(LikedProduct) private LikedProductRepository: typeof LikedProduct){}
  async create(createLikedProductDto: CreateLikedProductDto) {
    const liked = await this.LikedProductRepository.create(createLikedProductDto)
    return liked;
  }

  async findAll() {
    const cart = await this.LikedProductRepository.findAll({include:{all:true}});
    return cart;
  }
  ...
  async update(id: number, updateLikedProductDto: UpdateLikedProductDto) {
    const cart = await this.LikedProductRepository.update(updateLikedProductDto,{where:{id}})
    return cart;
  }
